...
public class testSymmetricGraph {

    public static void run(String algorithm, int nV, int nE, int seed){

        String graphType = "Symmetric graph";
        String file = "symmetricResults.csv";
        ...
        OrientableEdge randomEdge = (OrientableEdge) graph.edgeSet().toArray()[random.nextInt(graph.edgeSet().size())];
        graph.addEdge(graph.getEdgeTarget(randomEdge), graph.getEdgeSource(randomEdge));
